...
void doubleAtomicSprocket() {
  ...
  for (int i = 0; i < fft.specSize() ; i++) {
    ...
    rect(0, 0, fft.getBand(i)/20+fft.getFreq(i)/15,fft.getBand(i)/20+fft.getFreq(i)*(float(mouseY/50)));
    popMatrix();
  }
  popMatrix();
  pushMatrix();
  ...
  for (int i = 0; i < fft.specSize() ; i++) {
    ...
    rotateX(sin(angle[i]/3));
    rotateY(cos(angle[i]/3));
    //    stroke(fft.getFreq(i)*2,0,fft.getBand(i)*2);
    // fill(255, 255-fft.getFreq(i)*2, 255-fft.getBand(i)*2);
    fill(255,30);
    pushMatrix();
    translate((x[i]+250)%width, (y[i]+250)%height);
    rect(0, 0, fft.getBand(i+1), fft.getBand(i+1)+fft.getFreq(i)*(mouseX/100));
    popMatrix();
  }
  popMatrix();
